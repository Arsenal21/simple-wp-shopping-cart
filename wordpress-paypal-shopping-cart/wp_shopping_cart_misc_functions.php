...
function wpc_append_values_to_custom_field( $name, $value ) {
    ...
    $custom_field_val	 = $sess->get_data( 'wp_cart_custom_values' );
    $new_val		 = $name . '=' . $value;
    ...
    $sess->set_data( 'wp_cart_custom_values', $custom_field_val );
...
function wpspc_insert_new_record() {
	...
	$sess			 = WPSPSCSessions::get_instance();
	$sess->set_data( 'simple_cart_id', $post_id );
	...
	if ( isset( $simpleCart ) && ! empty( $simpleCart ) ) {
	    update_post_meta( $post_id, 'wpsc_cart_items', $simpleCart );
	}
    }
}
...
function wpspc_update_cart_items_record() {
    ...
    $simpleCart	 = $sess->get_data( 'simpleCart' );
    if ( isset( $simpleCart ) && ! empty( $simpleCart ) ) {
	$post_id = $sess->get_data( 'simple_cart_id' );
	update_post_meta( $post_id, 'wpsc_cart_items', $simpleCart );
    }
}
