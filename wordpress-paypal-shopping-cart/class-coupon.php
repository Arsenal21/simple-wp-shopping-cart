...
function wpspsc_apply_cart_discount( $coupon_code ) {
	...
	$sess->set_data( 'wpspsc_cart_action_msg', '<div class="wpspsc_error_message">' . __( "Coupon code used does not exist!", "wordpress-simple-paypal-shopping-cart" ) . '</div>' );
	return;
    }
	...
	if ( $current_date > $coupon_expiry_date ) {
	    $sess->set_data( 'wpspsc_cart_action_msg', '<div class="wpspsc_error_message">' . __( "Coupon code expired!", "wordpress-simple-paypal-shopping-cart" ) . '</div>' );
	    return;
	}
    }
    $wpspsc_discount_applied_once = $sess->get_data( 'wpspsc_discount_applied_once' );
    if ( isset( $wpspsc_discount_applied_once ) && $wpspsc_discount_applied_once == '1' ) {
	$sess->set_data( 'wpspsc_cart_action_msg', '<div class="wpspsc_error_message">' . __( "Discount can only be applied once per checkout!", "wordpress-simple-paypal-shopping-cart" ) . '</div>' );
	return;
    }
    ...
    $products	 = $sess->get_data( 'simpleCart' );
    $discount_total	 = 0;
    ...
    $sess->set_data( 'simpleCart', $products );
    $disct_amt_msg = print_payment_currency( $discount_total, $curr_symbol );
    $sess->set_data( 'wpspsc_cart_action_msg', '<div class="wpspsc_success_message">' . __( "Discount applied successfully! Total Discount: ", "wordpress-simple-paypal-shopping-cart" ) . $disct_amt_msg . '</div>' );
    $sess->set_data( 'wpspsc_discount_applied_once', '1' );
    $sess->set_data( 'wpspsc_applied_coupon_code', $coupon_code );
}
...
function wpspsc_reapply_discount_coupon_if_needed() {
    ...
    $wpspsc_discount_applied_once	 = $sess->get_data( 'wpspsc_discount_applied_once' );
    if ( isset( $wpspsc_discount_applied_once ) && $wpspsc_discount_applied_once == '1' ) {
	$coupon_code = $sess->get_data( 'wpspsc_applied_coupon_code' );
	$sess->set_data( 'wpspsc_discount_applied_once' );
