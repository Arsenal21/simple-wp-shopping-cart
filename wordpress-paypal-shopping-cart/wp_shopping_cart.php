...
function paypal_smart_checkout_scripts()
{	
	
	global $post;

	//only run if smartcheckout option is selected
	if(!get_option( 'wpspc_enable_pp_smart_checkout' ))
	{
		return;
	}

	if($post && has_shortcode(get_the_content(),"show_wp_shopping_cart")):
	?>
	<script src="https://www.paypalobjects.com/api/checkout.js"></script>
	<script>
		function render_paypal_buttons(
			

  return_url,
  pm_str,
  paypal_smartcheckout_evn,
  btn_layout,
  btn_size,
  btn_shape,
  btn_color,
  formatted_totalpluspostage,
  paypal_currency,
  formatted_total,
  formatted_postage_cost,
  items_list,
  carts_cnt,
  test_client_id,
  live_client_id
  ...
  paypal.Button.render({
    ...
    funding: {
      allowed: pm_str,	//["card","credit","elv"]
      disallowed: []
    },
    client: {
      sandbox: test_client_id,
      ...
      return actions.payment.create({
        payment: {
          transactions: [
            {
              amount: {
                total: formatted_totalpluspostage,
                currency: paypal_currency,
                details: {
                  subtotal: formatted_total,
                  shipping: formatted_postage_cost
                }
              },
              item_list: {
                items: items_list
    ...
    onError: function (error) {
      console.log(error);
      alert("Error occurred during PayPal Smart Checkout process.\n\n" + error);
    },
      ...
      return actions.payment.execute().then(function (data) {
        jQuery.post(
          ...
          .fail(function (result) {
            ...
            jQuery(".wpspsc-spinner-cont").hide();
            alert("HTTP error occurred during payment process: " + result.status + " " + result.statusText);
          });
      });
    }
...
add_action( 'wp_head', 'paypal_smart_checkout_scripts', 1 );
